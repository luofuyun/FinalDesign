...
/*---------for IE 5.x bug*/ 
html>body td,th{ font-size:15px;} 
</style> 
<style type="text/css" >
.recentcomments a {
...
<link rel='stylesheet' id='custom-style-css' href='css/techManageStudent.css'
	type='text/css' media='all' />
...
<script type='text/javascript' src='../js/teachManageStudent.js'></script>
<script type="text/javascript">
</script>
<script type="text/javascript">
	setInterval(
			"showtime.innerHTML=new Date().toLocaleString()+' 星期'+'日一二三四五六'.charAt(new Date().getDay());",
			1000);
...
<div>
	<div id="submits" class="ui-datatable ui-widget">
		...
		<div style="margin-top:10px;margin-bottom:10px;margin-left:5px;">
				        开设课程 : &nbsp;<select name="j_idt44" size="1" id="courseforteacher" style="width:40" onchange="onCourseSelectChange(this);">					
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="text" id="searchusername" value="输入用户名查询用户" style="color: gray;"
			onfocus="if(value=='输入用户名查询用户') {value='';}"
			onblur="if (value=='') {value='输入用户名查询用户';}"></input>
					<button class="btn btn-primary"  onclick="searchStudentByName()">查询</button>
					<button class="btn btn-primary" onclick="toAddUser()">添加新用户</button>
					<button class="btn btn-primary" onclick="toBatchAddUser()">批量添加新用户</button>
		            <button class="btn btn-primary" onclick="BatchDelUser()">删除所选用户</button>
			...
			<table id="mytable" cellspacing="0"
				...
				<tr align="center">
					...
					<th scope="col" abbr="Dual 1.8" align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;学&nbsp;&nbsp;号</th>
					<th scope="col" abbr="Dual 1.8">&nbsp;&nbsp;姓名</th>
					<th scope="col" abbr="Dual 2">&nbsp;&nbsp;教学班</th>
					<th scope="col" abbr="Dual 2.5">&nbsp;&nbsp;所属学院</th>
					<th scope="col" abbr="Dual 2.0">&nbsp;年级</th>
					<th scope="col" abbr="Dual 2.5">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;专业</th>
					<th scope="col" abbr="Dual 2.5">&nbsp;&nbsp;&nbsp;班级</th>
					<th scope="col" abbr="Dual 2">&nbsp;&nbsp;&nbsp;联系方式</th>
					<th scope="col" abbr="Dual 2">&nbsp;&nbsp;&nbsp;&nbsp;操&nbsp;&nbsp;作</th>
				</tr>
				<c:forEach  var="tempuser" items="${userList}" >
	              <tr align="center">
	                 <th scope="row" abbr="Model" class="spec"><input
						type="checkbox" id="selectUser" name="selectUserCk" value="${tempuser.user.accountId}"/></th>
	                  <td>${tempuser.user.accountId}</td>
	                  <td><a href="javascript:toEditStudent('${tempuser.teachclass.classId}','${tempuser.user.userId}')" title="编辑信息">${tempuser.user.userName}</a></td>
	                  <td>${tempuser.teachclass.className}</td>
	                  <td>${tempuser.user.college}</td>
	                  <td>${tempuser.user.grade}</td>
	                  <td>${tempuser.user.department}</td>
	                  <td>${tempuser.user.adminclass.className}</td>
	                  <td>${tempuser.user.contact}</td>
	                  <td>
	                    <button class="btn btn-primary" onclick="resetPassword('${tempuser.user.userName}','${tempuser.user.userId}')">重置密码</button>
						<button class="btn btn-primary" onclick="delUserInCourse('${tempuser.user.userId}','${tempuser.user.userName}','${tempuser.teachclass.classId}','${tempuser.teachclass.className}')">删除</button>           
	                  </td>
	                  </tr>
	          </c:forEach>	 
			</table>
