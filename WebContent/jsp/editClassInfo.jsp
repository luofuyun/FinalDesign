...
<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/editClassInfo.js"></script>
...
<body>
	...
	<div class="container">
		...
		<h4>
			<i class="fa fa-list-ul"></i>当前位置 ：填写班级信息
		</h4>
		<section class="well">
			<form id="form">
				<table style="font-size: 15px;" cellpadding="5" cellspacing="5">
					<tr>
						<td><input type="hidden" id="classId"
							name="classId"
							value="${classForShow.classId}" /></td>
					</tr>
					<tr>
						<td><input type="hidden" id="courseId"
							name="courseId"
							value="${courseId}" /></td>
					</tr>
					<h2>
					...
					<tr>
						...
						<td><input type="text" id="className" name="className"
							value="${classForShow.className}" maxLength="15" /></td>
						<td><div id="classnameerror"
					...
					<tr>
						...
						<td><select id="classType" name="classType">
								<c:choose>
									<c:when test="${classForShow.type==1}">
										<option value='1' selected="selected">教学班</option>
										<option value='2'>行政班</option>
									</c:when>
									<c:otherwise>
										<option value='1'>行政班</option>
										<option value='2' selected="selected">教学班</option>
									</c:otherwise>
								</c:choose>
						</select></td>
					</tr>
					<tr>
						<td></td>
						<td><button id="save_button" name="save_button"
								class="btn btn-primary" onclick="modifyClassInfo()"
								type="submit"> 保存 </button>&nbsp;&nbsp; <button id="cancel_button"
								name="cancel_button" class="btn btn-primary" onclick="cancel()"
								type="submit"> 取消 </button></td>
					</tr>
				</table>
				</form>
		</section>
